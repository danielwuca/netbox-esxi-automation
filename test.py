...
def get_vm_obj_netbox():
    ...
    for vm in vms:
        vm_obj = {
            ...
            "memoryGB": vm.memory // 1024,  # Assuming memory is in MB in NetBox
            "numCPUs": vm.vcpus,
            "disk_size_gb": vm.disk // 1024,  # Assuming disk size is in MB in NetBox
...
def create_vm_vsphere(vm_name, memoryGB, numCPUs, cluster_name, datastore_name, disk_size_gb, network_name, tags, os_name):
...
def main():
    ...
    for vm in vms:
        ...
        create_vm_vsphere(
            ...
            datastore_name=DATASTORE_NAME,  # Assuming a single datastore for all VMs
            disk_size_gb=vm['disk_size_gb'],
            network_name=NETWORK_NAME,  # Assuming a single network for all VMs
            tags=vm['tags'],
            os_name=vm['os_name']
        )
